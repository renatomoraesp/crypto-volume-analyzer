...
const createTweet = (pair, pumpIndex, priceDifference, minutes, s) => {
  if (priceDifference > 0) {
    return 'PUMP ALERT: ' + '$' + pair.split('/')[0] +' / ' + '$' + pair.split('/')[1] + ' volume has increased by ' + pumpIndex + ' in the last ' + minutes + 'minute' + s + '. Price is up ' + priceDifference + '%.';
  }
  return 'DUMP ALERT: ' + '$' + pair.split('/')[0] + ' / ' + '$' + pair.split('/')[1] + ' volume has increased by ' + pumpIndex + ' in the last minute. Price is down ' + priceDifference + '%.';
}
